...
- do this only if ***your file doesn't have execute perm***, `chmod u+x docker-installer.sh`
![File perm w x](./assets/image-2.png)

- run the script using `./docker-installer.sh`

- Hurrah :tada: now official docker package is installed in the desired system. you can check everything works fine by running `sudo docker run hello-world`
...
![user-interaction](./assets/image-3.png)

whereas, for ***non-interactive*** purpose you can run the script in following way `bash docker-installer.sh --non-interactive`
